(function () {
  ...
  const initTimer = function () {
    ...
    const intervalInfoContEl = document.getElementById(
      "interval-info-container"
    );
    ...
    const currentTheme = localStorage.getItem("intervalAppTheme")
      ? localStorage.getItem("intervalAppTheme")
      : null;
    let intervals = [];
    let intervalIndex = 1;
    let workout;
    ...
    function retrieveInts() {
      ...
      overallCount = 0;
      totalTime =
        workout?.intervals?.reduce((prev, num) => {
    ...
    function arcMove(oneInterval, index) {
      ...
      arcInterval = setInterval(function () {
        ...
        ctx.arc(
          ...
          120,
          (Math.PI / 180) * 270,
          (Math.PI / 180) * (270 + overallDegrees)
        );
        ctx.stroke();
